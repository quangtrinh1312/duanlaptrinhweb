...
public class DanhSachSinhVienDAO {
	...
	public void DeleteDoanVien(String id) {
		connection=ConnectDB.getConnection();
		String sql="DELETE FROM `cuoikitkltweb`.`chitietki` WHERE  (`maSinhVien` = ?);";
	...
	public void DeleteDoanVien1(String id) {
		connection=ConnectDB.getConnection();
		String sql="DELETE FROM `cuoikitkltweb`.`chitietki` WHERE  (`maSinhVien` = ?);";
		String sql1="DELETE FROM `cuoikitkltweb`.`doanvien` WHERE (`maSinhVien` = ?);";
		try(PreparedStatement delKi=connection.prepareStatement(sql);
	...
	public int getLastMsv() {
		connection=ConnectDB.getConnection();
		String sql="SELECT * FROM cuoikitkltweb.doanvien  order by maSinhVien desc limit 1";
		try {
			pS=connection.prepareStatement(sql);
			rS=pS.executeQuery();
			if(rS.next()) {
				return rS.getInt("maSinhVien");
			}
		} catch (SQLException e) {
			// TODO: handle exception
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(dao.getLastMsv());
	}
}
